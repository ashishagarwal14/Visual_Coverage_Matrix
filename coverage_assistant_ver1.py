...
fig = plt.Figure(figsize=(width / 170+3, height / 160+1))
...
class Window(Frame):
        ...
        for j in range(1, p_col+1 ,1):
            for i in range(1, p_row+1, 1):
                if not(str(arr1[i-1][j-1])=="N"):
                    ...
                    x1.append(i)
                    y1.append(j)
                    annots = ax.annotate("", xy=(0, 0), xytext=(-22, -25), textcoords="offset points", fontsize=5)
                    annots.xy = (j, i)
    ...
    def full_Cov(self, ar):
        ...
        for j in range(0, row, 1):
            ...
            print(str(x1))
            print(str(y1))
            self.line, = ax.plot(y1, x1, linestyle='dashed', linewidth=1, marker='.', markerfacecolor='blue',
